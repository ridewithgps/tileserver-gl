...
from tests.tile_coords import DATA_RWGPS, tiles_for, rendered_url
...
RENDERED_TILES = tiles_for(DATA_RWGPS)


def build_paths(style: str, fmt: str) -> list[str]:
    return [rendered_url(style, t, fmt=fmt) for t in RENDERED_TILES]
...
class TestBenchmark:
    ...
    def test_benchmark_png(self, base_url, client, duration, concurrency, perf_results, style):
        baselines = load_baselines()["rendered_png"]
        paths = build_paths(style, "png")
        warmup(client, paths)
        ...
        print(f"\nPNG ({style}): {tiles_per_sec:.1f} tiles/s, p50={pcts[50]:.0f}ms p95={pcts[95]:.0f}ms p99={pcts[99]:.0f}ms")
    ...
    @pytest.mark.timeout(0)
    def test_benchmark_webp(self, base_url, client, duration, concurrency, perf_results, style):
        baselines = load_baselines()["rendered_webp"]
        paths = build_paths(style, "webp")
        warmup(client, paths)
        ...
        print(f"\nWebP ({style}): {tiles_per_sec:.1f} tiles/s, p50={pcts[50]:.0f}ms p95={pcts[95]:.0f}ms p99={pcts[99]:.0f}ms")
